...
import { AlertCircle, File as FileIcon, Trash2, Download, Eye, Loader2, Upload } from 'lucide-react';
    ...
    <DialogContent className="max-w-md lg:max-w-2xl xl:max-w-4xl h-[90vh] flex flex-col">
      ...
      <div className="flex-grow flex items-center justify-center bg-muted/50 rounded-lg overflow-auto relative">
        {mimeType.startsWith('image/') ? (
          // For base64 only! If you want to preview ArrayBuffer, you need to createObjectURL
...
function SingleUploader({
    ...
    async (selectedFile: File | null) => {
      ...
      if (maxSize && selectedFile.size > maxSize) {
        toast({
          ...
          description: `The file size cannot exceed ${formatFileSize(maxSize)}.`
        });
        return;
      }
  ...
  const fileSrc = useMemo(() => {
    ...
    // ArrayBuffer needs conversion for preview
    return null;
  }, [value]);
  ...
  const fileTypeIconProps = useMemo(() => {
    switch (fileType) {
      ...
      case 'audio':
        return { icon: FileAudioIcon, className: 'text-orange-500' };
      case 'video':
        return { icon: FileVideoIcon, className: 'text-blue-500' };
      default:
            ...
            <div className="flex-shrink-0">
              <TooltipProvider>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <DialogTrigger asChild>
                          <Button size="icon" variant="ghost" onClick={() => setViewerOpen(true)}>
                            <Eye />
                          </Button>
                        </DialogTrigger>
                      </TooltipTrigger>
                ...
                {fileSrc && (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <a href={fileSrc} download={value.fileInfo.name}>
