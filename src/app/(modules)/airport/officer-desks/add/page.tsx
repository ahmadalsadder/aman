...
export default function AddAirportOfficerDeskPage() {
    ...
    const canCreate = hasPermission(['airport:desks:create']);
        ...
        const fetchData = async () => {
            ...
            try {
                const [portsRes, terminalsRes, zonesRes, workflowsRes, riskProfilesRes] = await Promise.all([
                    api.get<Port[]>('/data/ports?moduleType=airport'),
    ...
    const handleSave = async (formData: OfficerDeskFormValues) => {
        ...
        if (result.isSuccess) {
            ...
            router.push('/airport/officer-desks');
        } else {
            toast({
        ...
        <div className="space-y-6">
             <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink href="/airport/officer-desks" icon={Monitor}>{tNav('officerDesks')}</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
